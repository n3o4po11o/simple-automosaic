...
取 `models-standard.zip`（或 `.tar.zst`），**解压到以下目录**（使目录内直接是
`manifest.json` + `*.onnx`）：
...
- `archive` 档 M5 ensemble 组件（GDino/SAM2.1/RetinaFace/OSNet + 1536 主检，~1.6GB）不在
  一键包内，但同样在 models release 逐文件提供（`direct_url` 为上游直链，可对照）。
...
| `simple-automosaic-m5-models.exe` + `.sha256` | archive 档 M5 组件补充包（~1.6GB；五档全集 ~2.3GB 超 GitHub 资产 2GiB / NSIS 2GB 上限故拆分） | 同上 |
...
- **Windows 构建**（CI：windows-latest；本地同）：`pwsh scripts/build_windows.ps1`
  出三产物——zip（app+ffmpeg+运行库，无模型）/ portable 单文件（四档模型内置，
  NSIS 静默自释放到 `%LOCALAPPDATA%\SimpleAutoMosaic\app`，版本标记跳过重复解压）/
  M5 组件补充包单文件。**Windows 构建尚未真机验证**。
