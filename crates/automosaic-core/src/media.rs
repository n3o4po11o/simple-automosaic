...
pub fn list_hwaccels() -> Vec<String> {
    let Ok(out) = Command::new(&tool_path("ffmpeg"))
...
fn smoke_decode_run(input: &Path, hwaccel: &str) -> bool {
    let Ok(mut child) = Command::new(tool_path("ffmpeg"))
...
/// 依赖）；macOS 恒 Apple（统一显存，链无需重排）；Windows 的 DXGI 枚举
/// 随 Windows 适配（M-后置）一并做，当前返回 None 走默认链。
/// 多卡混布不同厂商时返回 None（保持保守的 vaapi 优先链）。
...
pub fn encode_cmd(output: &Path, o: &EncodeOptions) -> Vec<String> {
    ...
    match o.encoder.as_str() {
        ...
        e if e.ends_with("qsv") => {
            c.extend(
                ["-global_quality", "22", "-look_ahead", "1", "-extbrc", "1"]
                    .map(String::from),
            );
        }
