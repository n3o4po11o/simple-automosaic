...
pub fn backend_desc(device: &str) -> String {
    ...
    // Windows：auto 与显式 openvino 同径（2026-08-23 起默认 OpenVINO；
    // DML 实测慢 4 倍弃用；运行期不可用自动回退 CPU 由管线承接）
    #[cfg(target_os = "windows")]
    if device != "cpu" && device != "directml" {
        return "OpenVINO（Intel GPU/CPU）".into();
    }
    if device == "webgpu" {
        return "WebGPU（Dawn/Vulkan，实验）".into();
    }
...
/// Windows 推理 EP 统一选择宏（三变体按编译期 feature）。
/// SessionBuilder 是 ort 私有类型无法命名——宏原地修改避开类型签名。
/// 所有模型加载点共用，避免五处 cfg 块漂移。
#[cfg(target_os = "windows")]
#[macro_export]
macro_rules! windows_ep {
    ($b:expr, $device:expr) => {
        if $device != "cpu" {
            #[cfg(feature = "win-cuda")]
            {
                $b = $b
                    .with_execution_providers([ort::ep::CUDA::default().build()])
                    .unwrap_or_else(|e| e.recover());
            }
            #[cfg(feature = "win-directml")]
            {
                $b = $b
                    .with_execution_providers([ort::ep::DirectML::default().build()])
                    .unwrap_or_else(|e| e.recover());
            }
            #[cfg(not(any(feature = "win-cuda", feature = "win-directml")))]
            {
                $b = $b
...
fn commit_session(device: &str, model: &Path) -> Result<ort::session::Session, DetectError> {
    ...
    // OpenVINO EP（Windows 默认，2026-08-23 起）：Intel iGPU 的正解；
    // DirectML 在 Gen9.5 实测 2.2fps vs CPU 8.7fps（慢 4 倍）且处理管线
    // 真机崩溃（DmlCommandRecorder），弃用。运行时 = 官方 onnxruntime
    // win-x64 包（load-dynamic，dll 随应用分发）。非 Intel 机器 EP 创建
    // 失败由暖机回退 CPU 承接。
    #[cfg(target_os = "windows")]
    windows_ep!(b, device);
    ...
    // GPU EP 运行期兼容探测（真机教训：session 建立成功 ≠ 首帧能跑——
    // DML 曾在 Gen9.5 报 DXGI_ERROR_DEVICE_REMOVED；OpenVINO 同理可能驱动
    // 级失败）。零张量暖机一次，失败降 CPU EP 重建，"永远能跑"的运行期一环
    #[cfg(target_os = "windows")]
    if device != "cpu" {
        let mut session = b.commit_from_file(model)?;
        if dml_warmup_ok(&mut session) {
            return Ok(session);
        }
        eprintln!("warn: GPU EP 暖机失败，回退 CPU（device={device}）");
        return Ok(ort::session::Session::builder()?.commit_from_file(model)?);
    }
    Ok(b.commit_from_file(model)?)
}

/// DML 暖机：按各输入声明形状（动态维取 1）构造全零 f32 张量各喂一次。
/// 形状未知/构造失败/超大形状一律视为"通过"（交由真实推理验证），
/// 只有执行期明确报错才判失败——避免误降级。
#[cfg(target_os = "windows")]
fn dml_warmup_ok(session: &mut ort::session::Session) -> bool {
    use std::borrow::Cow;
    // 先拷出输入元数据：inputs() 的不可变借用不能与 run 的可变借用共存
    //（E0502，Windows 真机构建实测）
    let mut inputs: Vec<(String, Vec<i64>)> = Vec::new();
    for input in session.inputs() {
        let Some(shape) = input.dtype().tensor_shape() else {
            return true; // 非张量/未知形状：不暖机，交由真实推理验证
        };
        let dims: Vec<i64> = shape.iter().map(|&d| if d < 0 { 1 } else { d }).collect();
        inputs.push((input.name().to_string(), dims));
    }
    let mut feed: Vec<(Cow<'_, str>, ort::session::SessionInputValue<'_>)> = Vec::new();
    for (name, dims) in inputs {
        ...
        let value = match ort::value::Tensor::from_array((dims, vec![0f32; len])) {
            Ok(v) => v,
            Err(_) => return true,
        };
