...
# 1536 为本脚本现场导出（ONNX 导出非字节确定），先回写新 sha 再统一校验——
# 与 export_models.sh 的"manifest 回写保一致"机制对齐
PY="${AUTOMOSAIC_PY:-python3}"
"$PY" - <<'EOF'
import json, hashlib, os
m = json.load(open("models/manifest.json"))
p = "models/yolo26x-seg-1536.onnx"
if os.path.exists(p):
    h = hashlib.sha256(open(p, "rb").read()).hexdigest()
    for e in m["models"]:
        if e["file"] == "yolo26x-seg-1536.onnx":
            e["sha256"] = h
    json.dump(m, open("models/manifest.json", "w"), indent=2)
    print(f"1536 sha 回写: {h[:16]}…")
EOF
