...
# Mach-O magic（大端读数；含 fat/反向）——非对象成员（如 ort 预编译库里
# 混入的 bitcode/元数据成员）跳过：新 Xcode 的 libtool 对 "not an object
# file" 从 warning 收紧为 error（本地旧 libtool 仅告警且链接不需要它们）
MACH_O_MAGICS = {
    0xFEEDFACE, 0xFEEDFACF,          # MH_MAGIC / MH_MAGIC_64
    0xCEFAEDFE, 0xCFFAEDFE,          # 小端反读
    0xCAFEBABE, 0xBEBAFECA,          # FAT_MAGIC 及反读
}


def is_mach_o(body: bytes) -> bool:
    return len(body) >= 4 and int.from_bytes(body[:4], "big") in MACH_O_MAGICS
...
def main() -> int:
    ...
    while off + 60 <= len(data):
        ...
        if not is_mach_o(body):
            print(f"skip non-mach-o member: {name} ({len(body)}B)")
            continue
