...
pub fn process_video(
    ...
    let hwaccel = resolve_hwaccel(Path::new(&input), &opts.hwaccel, &meta);
    ...
    let key = cache_key(&eff.body, use_face, &opts.device, eff.conf, eff.tta);
    ...
    for (i, enc) in encoders.iter().enumerate() {
        ...
        let (det, face) = match take_models(&key, &eff, &opts.device) {
            Ok(v) => v,
            Err(e) => return fail(e),
        };
        ...
        match result {
            ...
            Err(pipe::PipelineError::EncoderFailed { .. }) if i + 1 < encoders.len() => {
                ...
                log(format!("[回退] {last_err}，尝试 {} …", encoders[i + 1]));
...
/// hwaccel 选择 → 实际值。auto = 候选链逐个对真实流冒烟（DESIGN §3.3，
/// 结果按流规格缓存）；meta 由调用方传入（process_video 已 probe）。
fn resolve_hwaccel(input: &Path, choice: &str, meta: &media::VideoMeta) -> Option<String> {
    match choice {
        "none" => None,
        "auto" => media::decode_chain()
            .into_iter()
            .flatten()
...
pub fn archive_render(
    ...
    let _ = sink.add(ProcessEvent::StageEnter { stage: ProcessStage::Inferring });
    let hwaccel = resolve_hwaccel(Path::new(&input), &hwaccel, &meta);
