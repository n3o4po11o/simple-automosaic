...
class DraggableAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DragToMoveArea(
      child: AppBar(
        title: title,
        // Linux/Windows：TitleBarStyle.hidden 移除系统装饰，窗口控件自绘在
        // 标题栏尾部；macOS 有悬浮交通灯无需自绘（Windows 首验补齐，2026-08-23）
        actions: [...?actions, if (!Platform.isMacOS) const WindowControls()],
...
/// Linux/Windows 自绘窗口控件（min/max/close，挂在 DraggableAppBar 尾部）：
/// TitleBarStyle.hidden 在 GTK 移除全部装饰、Windows 同样去框；仅 macOS
/// 保留悬浮交通灯不自绘。close 经 setPreventClose 走 onWindowClose（保存
/// 几何后再 destroy）。
