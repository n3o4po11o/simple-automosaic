...
class _ConfigScreenState extends State<ConfigScreen> {
  // auto-copy：硬解后拷回系统内存渲染（swscale 转色）。AppImage 环境（2026-08-22
  // 真机）libva 交还宿主后 mpv 硬解激活，但其 EGL interop 路径色彩错乱；
  // auto-copy 绕开 interop，保留硬解收益
  ...
  void initState() {
    super.initState();
    // 硬解后拷回系统内存渲染（swscale 转色）：AppImage 环境（2026-08-22 真机）
    // libva 交还宿主后 mpv 硬解激活，但其 EGL interop 路径色彩错乱；
    // auto-copy 绕开 interop，保留硬解收益
            ...
            child: Card(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Text(
                    switch (_device) {
                      'gpu' => Platform.isMacOS
                          ? S.t('CoreML：CPU + GPU（算子兼容性最稳）',
                              'CoreML: CPU + GPU (most compatible)')
                          : S.t(
                              'GPU 推理（Intel 集显 OpenVINO / N 卡 CUDA /'
                              ' A 卡 DirectML，按硬件自动选择）',
                              'GPU inference (Intel iGPU OpenVINO / NVIDIA '
                              'CUDA / AMD DirectML, auto-selected)'),
                      ...
                      'cpu' => Platform.isLinux
                          ...
                          : Platform.isWindows
                              ? S.t('ONNX Runtime CPU（不使用 OpenVINO）',
                                  'ONNX Runtime CPU (no OpenVINO)')
                      ...
                      _ => Platform.isLinux
                          ...
                          : Platform.isWindows
                              ? S.t(
                                  '自动选择最优 GPU 后端（Intel 集显 OpenVINO /'
                                  ' N 卡 CUDA / A 卡 DirectML），不可用回退 CPU',
                                  'Auto-selects best GPU backend (Intel iGPU '
                                  'OpenVINO / NVIDIA CUDA / AMD DirectML), '
                                  'falls back to CPU')
